    ...
    public SignupRequest deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException {
        ...
        if (roleNode.isArray()) {
            for (JsonNode role : roleNode) {
                ...
                } else if (ERole.ROLE_BUYER.toString().equalsIgnoreCase(roleValue)) {
                    roles.add(ERole.ROLE_BUYER.toString());
                } else if (ERole.ROLE_SELLER.toString().equalsIgnoreCase(roleValue)) {
                    roles.add(ERole.ROLE_SELLER.toString());
                }
            }
